...
form <- ~ EngDispl + NumCyl + Transmission + AirAspirationMethod + NumGears + TransLockup + TransCreeperGear + DriveDesc + IntakeValvePerCyl + ExhaustValvesPerCyl + CarlineClassDesc + VarValveTiming + VarValveLift
 ...
 # Your work here
 fit.min <- lm( FE ~ 1, fe )
 fit.min.forward <-stepAIC( fit.min, score = ~ 1, direction = "forward")
 
 
 fit.full <- lm( FE ~ ., fe )
 fit.full.backward <-stepAIC( fit.full, score = ~ 1, direction = "backward")
 
 fit.min.forward %>% summary()
 fit.full.backward %>% summary()
```

- Are they the same model? If not why?  Which is better? JUstify your answer.
- They are the same model, the linear model. The full model takes into accont of every feature values, which overfits the data, that results in less predicatability. The simplist approach fits better in predicting a base-line clean and fast.
...
```{r}
fit.min <- glm(formula = Class ~ 1, family = binomial, data = GermanCredit)
fit.min.forward <-stepAIC( fit.min, score = ~ 1, direction = "forward")
fit.min.forward %>% summary()
```
...
results <- rfe(GermanCredit[,-10], GermanCredit[,10], rfeControl=control)
print(results)

fit.full <- glm( formula = Class ~ CheckingAccountStatus.none +  CheckingAccountStatus.lt.0 + Duration + CreditHistory.Critical + Amount, family = binomial, data = GermanCredit )
fit.full.backward <-stepAIC( fit.full, score = ~ 1, direction = "backward")
fit.full.backward %>% summary()
```
